...
| Prepared headers and member/service records | Final RAR5/7 member and service images retained for single-archive output have an optional shared `with_max_prepared_header_bytes` capacity quota. Serializer scratch and block descriptors remain outside it. RAR5/7 service payloads borrow writer input; preparation makes no additional plaintext or ciphertext payload copy. Upstream input construction may still allocate. |
