...
| Prepared headers and member/service records | Final RAR5/7 member and service images retained for single-archive output have an optional shared `with_max_prepared_header_bytes` capacity quota. Fixed-size framing uses stack scratch; variable-length record buffers and block descriptors remain outside the quota. RAR5/7 service payloads borrow writer input; preparation makes no additional plaintext or ciphertext payload copy. Upstream input construction may still allocate. |
...
| Encryption | Single-archive member and service data is encrypted during emission in chunks. Volume preparation writes ciphertext into an additional spool before splitting. Keys, cipher state, padded headers and the bounded encryption buffer also allocate storage. |
